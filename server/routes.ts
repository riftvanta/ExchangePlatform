 ...
 */
router.post('/wallet', isAuthenticated, (async (req: Request, res: Response) => {
  try {
 ...
 */
router.post('/deposit/usdt', isAuthenticated, (async (req: Request, res: Response) => {
  try {
    const userId = req.session.userId as string;
    ...
    const insertedTransaction = await db.insert(transactions).values(newTransaction).returning();
 ...
 */
router.get('/admin/deposits', isAuthenticated, isAdmin, (async (req: Request, res: Response) => {
  try {
    const pendingDeposits = await db.select().from(transactions).where(eq(transactions.status, 'pending'));
 ...
 */
router.post('/admin/deposits/:transactionId/approve', isAuthenticated, isAdmin, (async (req: Request, res: Response) => {
  try {
    const { transactionId } = req.params;

    // Find the transaction
    const transaction = await db.select().from(transactions).where(eq(transactions.id, transactionId)).then(rows => rows[0] ?? null);
    ...
    const wallet = userWallets.find((w) => w.currency === transaction.currency);
    ...
    await db.update(transactions).set({ status: 'approved', updatedAt: new Date() }).where(eq(transactions.id, transactionId));
 ...
 */
router.post('/admin/deposits/:transactionId/reject', isAuthenticated, isAdmin, (async (req: Request, res: Response) => {
  try {
    const { transactionId } = req.params;
    ...
    await db.update(transactions)
      .set({ status: 'rejected', updatedAt: new Date(), rejectionReason: rejectionReason }) // Set rejectionReason
 ...
 */
router.get('/admin/deposits/:transactionId/image', isAuthenticated, isAdmin, (async (req: Request, res: Response) => {
  try {
    const { transactionId } = req.params;
